...
for i in cycles: ## Cycle
    ...
    for j in range(runs): ## Run
        if with_opf:
            data = pd.read_csv(os.path.join(results,'sim_opf_{}.{}.log'.format(i, j+1)), header=None, delimiter='\t')
            data[0] = data[0]-data.loc[0,0]
            data.drop_duplicates(4, inplace=True)
            voltages = data[4]
            opf_hits = opf_hits + [(voltages[(voltages <= low)].count() + voltages[(voltages >= high)].count())]
        if with_pi:
            data = pd.read_csv(os.path.join(results,'sim_pi_{}.{}.log'.format(i, j+1)), header=None, delimiter='\t')
            data[0] = data[0]-data.loc[0,0]
            data.drop_duplicates(4, inplace=True)
            voltages = data[4]
...
ax.legend(tuple([pv_plot[0]]+[opf_bar[0] if with_opf else []]+[pi_bar[0] if with_pi else []]), tuple(['No Control']+['OPF Control' if with_opf else []]+ ['PI Control' if with_pi else []]))
