...
cpu: list = []
for i in nodes:
    data = pd.read_csv('./ps.{}.out'.format(i), skiprows=1, header=None, delimiter=r"\s+")
...
ax_mem.set_ylabel("Average Memory\nConsumption (MiB)", color="blue", fontsize=12)
ax_mem.set_ylim(150, 200)
ax_mem.xaxis.set_tick_params(labelsize=12)
ax_mem.yaxis.set_tick_params(labelsize=12, colors='blue')
...
ax_cpu.set_yticks([i for i in arange(int(min([min(m) for m in cpu])),int(max([max(m) for m in cpu])), 10)])
ax_cpu.set_ylabel('Average CPU\nUsage (%)', color="red", fontsize=12)
ax_cpu.set_ylim(95, 120)
ax_cpu.xaxis.set_tick_params(labelsize=12)
ax_cpu.yaxis.set_tick_params(labelsize=12, colors='r')

plt.savefig('mem_cpu.png', figsize=figsize, dpi=600)
plt.show()
